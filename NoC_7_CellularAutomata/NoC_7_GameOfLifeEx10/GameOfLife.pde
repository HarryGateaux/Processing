    ...
    if (keyPressed == true) {
      ...
      for (int row = 0; row < rows; row++) {
        for (int col = 0; col < cols; col++) {
          ...
          //ruleset
          if      ((board[row][col].state == 1) && (neighbours_alive <  2)) {if(random(100) < 80) {board[row][col].newState(0);}} // 80 pct chance of dying if less than 2 neighbours alive
          else if ((board[row][col].state == 1) && (neighbours_alive >  3)) {if(random(100) < 60) {board[row][col].newState(0);}} // 60 pct chance of dying if more than 3 neighbours alive
