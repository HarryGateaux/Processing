  ...
  void create() {
    ...
    for (int row = 0; row < rows; row++) {
      for (int col = 0; col < cols; col++) {
        if (row % 2 == 0) {
          board[row][col] = new Cell(col * w * 3, row * h, w, h);
        } else {

          board[row][col] = new Cell(col * w * 3 +  w + h/2, row * h, w, h);
        }
      }
    }
  }
    ...
    if (keyPressed == true) {
      ...
      for (int row = 0; row < rows; row++) {
        for (int col = 0; col < cols; col++) {
          ...
          neighbours_alive = 0;
          neighbours_alive += board[(row - 2 + rows) % rows][(col + cols) % cols].previous;
          neighbours_alive += board[(row - 1 + rows) % rows][(col - 1 + cols) % cols].previous;
          neighbours_alive += board[(row - 1 + rows) % rows][(col + 1 + cols) % cols].previous;
          neighbours_alive += board[(row + 1 + rows) % rows][(col - 1 + cols) % cols].previous;
          neighbours_alive += board[(row + 1 + rows) % rows][(col + 1 + cols) % cols].previous;
          neighbours_alive += board[(row + 2 + rows) % rows][(col + cols) % cols].previous;
...
//          }
          ...
          //ruleset
          if      ((board[row][col].state == 1) && (neighbours_alive <  1)) board[row][col].newState(0);
