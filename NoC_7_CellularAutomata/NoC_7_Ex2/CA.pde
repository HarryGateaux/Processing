...
class CA {
  ...
  //start again from the top with a random ruleset
  void reboot() {
    ...
    for (int i = 0; i < ruleset.length; i++) {
      //ruleset[i] = (random.nextBoolean()) ? 1 : 0; //casting the random bool to int
      ruleset[i] = int(random(1) * 2) ;
    }
  }
